...
def print_model_summary(model: ModelWrapper, label: str) -> None:
    ...
    for graph_input in model.graph.input:
        print(
            f"  input  {graph_input.name}: "
            f"shape={_get_tensor_shape(model, graph_input.name)} "
            f"dtype={model.get_tensor_datatype(graph_input.name)}"
        )
        for ctx in _quant_context_for_graph_input(model, graph_input.name):
            print(f"           quant_info={ctx}")
    for graph_output in model.graph.output:
        print(
            f"  output {graph_output.name}: "
            f"shape={_get_tensor_shape(model, graph_output.name)} "
            f"dtype={model.get_tensor_datatype(graph_output.name)}"
        )
...
def convert_qonnx_to_finn(
    ...
    # FINN can introduce unnamed nodes. Normalize names so later HLS stages
    # don't produce empty top-function identifiers such as top_.cpp.
...
def main(qonnx_in: str, finn_out: str | None):
    if finn_out is None:
...
if __name__ == "__main__":
    ...
    ap.add_argument("--qonnx_in", type=str, default="deeponet_u250_int4_qonnx.onnx")
    ap.add_argument("--finn_out", type=str, default=None)
    args = ap.parse_args()
    main(args.qonnx_in, args.finn_out)
